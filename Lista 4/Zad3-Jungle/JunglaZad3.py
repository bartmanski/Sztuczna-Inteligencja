...
class Jungle:
    ...
    def best_move(self, moves, player):
        bscore, bmove = -1, 0
        for move in moves:
            save = deepcopy((self.board, self.pieces, self.peace_counter))
            self.do_move(move)
            score = 0
            for piece in self.pieces[player].items():
                x, y = piece[1]
                dx, dy = self.dens[1 - player]
                dist = 12 - abs(dx - x) - abs(dy - y)
                score += dist
                # # sys.stderr.write("%d\n"%dist)
                if piece[0] == 7:
                    score += dist * 2
                else:
                    score += dist * (piece[0] + 1)
            self.board, self.pieces, self.peace_counter = save
