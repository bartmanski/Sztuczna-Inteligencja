...
class state:
    ...
    def min_niepewnosci(self):
        ...
        for i in range(few_random_moves_to_deacrease_pos):
            state = states[-1]
            if(i< few_random_moves_to_deacrease_pos//4):
                states.append(state.move_all(1,0,'D'))
            elif(i< 2*few_random_moves_to_deacrease_pos//4):
                states.append(state.move_all(-1,0,'U'))
            elif(i< 3*few_random_moves_to_deacrease_pos//4):
                states.append(state.move_all(0,1,'R'))
            else:
    ...
    def solve(self, end_positions, kolumny, wiersze, prepocessed_heuristic=None):
        ...
        print(len(start_state.positions))
        if(len(start_state.positions) > 20):
            start_state = self.min_niepewnosci()
            print(len(start_state.positions))
        queue = PriorityQueue()
        queue.put((0,id(start_state), start_state))
        visited = set()
        visited.add(hash(start_state.positions,kolumny, wiersze))
