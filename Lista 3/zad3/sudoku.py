...
def get_square(a, b):
    return [V(i, j) for i in range(3*a, 3*a+3) for j in range(3*b, 3*b+3)]

def squares():
    return [all_different(get_square(a, b)) for a in range(3) for b in range(3)]
...
"python3 validator3.py --stdio zad4 python3 sudoku.py"
